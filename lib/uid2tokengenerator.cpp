...
namespace uid2 {
    ...
    EncryptImpl(std::vector<std::uint8_t> &data, const std::uint8_t *iv, const std::vector<std::uint8_t> &secret) {
    ...
    std::string GenerateUid2TokenV2(const std::string &identity, const Key &masterKey, int siteId, const Key &siteKey,
    ...
    GenerateUid2TokenV3(const std::string &identity, const uid2::Key &masterKey, int siteId, const uid2::Key &siteKey,
                        EncryptTokenParams params) {
        return GenerateUID2TokenWithDebugInfo(identity, masterKey, siteId, siteKey, params,
                                              AdvertisingTokenVersion::V3);
    }
    ...
    GenerateUid2TokenV4(const std::string &identity, const uid2::Key &masterKey, int siteId, const uid2::Key &siteKey,
                        EncryptTokenParams params) {
        return GenerateUID2TokenWithDebugInfo(identity, masterKey, siteId, siteKey, params,
                                              AdvertisingTokenVersion::V4);
    }

    std::string
    GenerateUID2TokenWithDebugInfo(const std::string &uid, const Key &masterKey, int siteId, const Key &siteKey,
                                   EncryptTokenParams params, AdvertisingTokenVersion adTokenVersion) {
        ...
        const auto masterPayloadLen = masterPayloadWriter.GetPosition()
                                      + EncryptGCM(sitePayload, sitePayloadWriter.GetPosition(), siteKey.secret.data(),
                                                   masterPayload + masterPayloadWriter.GetPosition());

        std::vector<std::uint8_t> rootPayload(256);
        BigEndianByteWriter writer(rootPayload);

        char firstChar = uid[0];
        IdentityType identityType = (firstChar == 'F' || firstChar == 'B') ? IdentityType::Phone
                                                                           : IdentityType::Email; //see UID2-79+Token+and+ID+format+v3
        ...
        const auto rootPayloadLen = writer.GetPosition()
                                    + EncryptGCM(masterPayload, masterPayloadLen, masterKey.secret.data(),
                                                 rootPayload.data() + writer.GetPosition());
