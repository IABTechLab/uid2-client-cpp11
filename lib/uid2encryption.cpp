...
namespace uid2 {
    ...
    static void Decrypt(const std::uint8_t *data, int size, const std::uint8_t *iv, const std::uint8_t *secret,
                 ...
                 bool checkValidity)
    {
        ...
        bool isBase64UrlEncoding = std::any_of(headerStr.begin(), headerStr.end(),
                                               [](char c) { return c == '-' || c == '_'; });
        try {
                   ...
                   IdentityScope identityScope, bool checkValidity)
    {
        ...
        const int masterPayloadLen = DecryptGCM(reader.GetCurrentData(), reader.GetRemainingSize(),
                                                masterKey->secret.data(), masterPayload);
        ...
        const auto sitePayloadLen = DecryptGCM(masterPayloadReader.GetCurrentData(),
                                               masterPayloadReader.GetRemainingSize(), siteKey->secret.data(),
                                               sitePayload);
        ...
        const std::vector<std::uint8_t> identityBytes(sitePayloadReader.GetCurrentData(),
                                                      sitePayloadReader.GetCurrentData() +
                                                      sitePayloadReader.GetRemainingSize());
    ...
    EncryptUID(const std::string &uid, const KeyContainer &keys, Timestamp now, IdentityScope identityScope)
    {
        ...
        std::string encryptionResult = GenerateUid2TokenV4(uid, *masterKey, keys.getCallerSiteId(), *defaultKey,
                                                           encryptParams);
    ...
    EncryptData(const EncryptionDataRequest &req, const KeyContainer *keys, IdentityScope identityScope)
    {
        ...
        EncryptGCM(payload.data(), payload.size(), iv, key->secret.data(),
                   encryptedBytes.data() + writer.GetPosition());
                                              ...
                                              IdentityScope identityScope)
    {
        ...
        const auto payloadLen = DecryptGCM(reader.GetCurrentData(), reader.GetRemainingSize(), key->secret.data(),
                                           payload.data());
        ...
        return DecryptionDataResult::MakeSuccess(
                {payloadReader.GetCurrentData(), payloadReader.GetCurrentData() + payloadReader.GetRemainingSize()},
                encryptedAt);
    }
    ...
    int DecryptGCM(const std::uint8_t *encrypted, int size, const std::uint8_t *secret, std::uint8_t *out_decrypted)
    {
        ...
        int outLen = 0;
        if (!EVP_DecryptUpdate(ctx.get(), out_decrypted, &outLen, encrypted + GCM_IV_LENGTH,
                               size - (GCM_IV_LENGTH + GCM_AUTHTAG_LENGTH))) {
            throw std::runtime_error("failed to decrypt");
        }
        const int totalLen = outLen;

        if (!EVP_CIPHER_CTX_ctrl(ctx.get(), EVP_CTRL_GCM_SET_TAG, GCM_AUTHTAG_LENGTH,
                                 (void *) (encrypted + (size - GCM_AUTHTAG_LENGTH)))) {
