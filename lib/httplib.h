...
struct ci {
    bool operator()(const std::string& s1, const std::string& s2) const
    {
        return std::lexicographical_compare(
            s1.begin(), s1.end(), s2.begin(), s2.end(), [](unsigned char c1, unsigned char c2) { return ::tolower(c1) < ::tolower(c2); });
    }
};
...
struct Response {
    ...
    void
    set_chunked_content_provider(const char* content_type, ContentProviderWithoutLength provider, const std::function<void()>& resource_releaser = nullptr);
...
public:
    ...
    explicit Client(const char* scheme_host_port, const std::string& client_cert_path, const std::string& client_key_path);
...
template <typename T>
inline bool process_server_socket_core(socket_t sock, size_t keep_alive_max_count, time_t keep_alive_timeout_sec, T callback)
{
...
inline bool process_server_socket(
    ...
    T callback)
{
    return process_server_socket_core(sock, keep_alive_max_count, keep_alive_timeout_sec, [&](bool close_connection, bool& connection_closed) {
...
template <typename T>
inline bool
process_client_socket(socket_t sock, time_t read_timeout_sec, time_t read_timeout_usec, time_t write_timeout_sec, time_t write_timeout_usec, T callback)
{
    SocketStream strm(sock, read_timeout_sec, read_timeout_usec, write_timeout_sec, write_timeout_usec);
...
inline socket_t create_client_socket(
    ...
    Error& error)
{
    auto sock = create_socket(host, port, 0, tcp_nodelay, std::move(socket_options), [&](socket_t sock, struct addrinfo& ai) -> bool {
        if (!intf.empty()) {
#ifdef USE_IF2IP
            auto ip = if2ip(intf);
            if (ip.empty()) {
...
inline void get_remote_ip_and_port(const struct sockaddr_storage& addr, socklen_t addr_len, std::string& ip, int& port)
{
    ...
    if (!getnameinfo(
            reinterpret_cast<const struct sockaddr*>(&addr), addr_len, ipstr.data(), static_cast<socklen_t>(ipstr.size()), nullptr, 0, NI_NUMERICHOST)) {
        ip = ipstr.data();
    }
}
...
inline bool can_compress_content_type(const std::string& content_type)
{
    return (!content_type.find("text/") && content_type != "text/event-stream") || content_type == "image/svg+xml" ||
           content_type == "application/javascript" || content_type == "application/json" || content_type == "application/xml" ||
        ...
        if (decompressor) {
            if (decompressor->is_valid()) {
                ContentReceiverWithProgress out = [&](const char* buf, size_t n, uint64_t off, uint64_t len) {
                    return decompressor->decompress(buf, n, [&](const char* buf, size_t n) { return receiver(buf, n, off, len); });
                };
...
inline std::pair<std::string, std::string> make_basic_authentication_header(const std::string& username, const std::string& password, bool is_proxy = false)
{
...
inline void Response::set_content_provider(size_t in_length, const char* content_type, ContentProvider provider, const std::function<void()>& resource_releaser)
{
    assert(in_length > 0);
    set_header("Content-Type", content_type);
...
inline void Response::set_content_provider(const char* content_type, ContentProviderWithoutLength provider, const std::function<void()>& resource_releaser)
{
    set_header("Content-Type", content_type);
    content_length_ = 0;
...
inline void
Response::set_chunked_content_provider(const char* content_type, ContentProviderWithoutLength provider, const std::function<void()>& resource_releaser)
{
    set_header("Content-Type", content_type);
    content_length_ = 0;
    content_provider_ = detail::ContentProviderAdapter(std::move(provider));
...
inline Server::Server() : new_task_queue([] { return new ThreadPool(CPPHTTPLIB_THREAD_POOL_COUNT); }), svr_sock_(INVALID_SOCKET), is_running_(false)
{
#ifndef _WIN32
...
{
    get_handlers_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    post_handlers_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    post_handlers_for_content_reader_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    put_handlers_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    put_handlers_for_content_reader_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    patch_handlers_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    patch_handlers_for_content_reader_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    delete_handlers_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    delete_handlers_for_content_reader_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
{
    options_handlers_.push_back(std::make_pair(std::regex(pattern, pattern_len), std::move(handler)));
    return *this;
}
...
inline socket_t Server::create_server_socket(const char* host, int port, int socket_flags, SocketOptions socket_options) const
{
    return detail::create_socket(host, port, socket_flags, tcp_nodelay_, std::move(socket_options), [](socket_t sock, struct addrinfo& ai) -> bool {
        if (::bind(sock, ai.ai_addr, static_cast<socklen_t>(ai.ai_addrlen))) {
            return false;
        }
...
inline socket_t ClientImpl::create_client_socket(Error& error) const
{
    if (!proxy_host_.empty() && proxy_port_ != -1) {
        return detail::create_client_socket(
            proxy_host_.c_str(), proxy_port_, tcp_nodelay_, socket_options_, connection_timeout_sec_, connection_timeout_usec_, interface_, error);
    }
    return detail::create_client_socket(
        host_.c_str(), port_, tcp_nodelay_, socket_options_, connection_timeout_sec_, connection_timeout_usec_, interface_, error);
}
...
inline Result ClientImpl::Get(const char* path, ResponseHandler response_handler, ContentReceiver content_receiver, Progress progress)
{
    ...
    Progress progress)
{
    ...
    std::string path_with_query = detail::append_query_params(path, params);
    return Get(path_with_query.c_str(), params, headers, response_handler, content_receiver, progress);
}
...
inline Result ClientImpl::Post(const char* path, ContentProviderWithoutLength content_provider, const char* content_type)
{
...
inline Result ClientImpl::Patch(const char* path, ContentProviderWithoutLength content_provider, const char* content_type)
{
...
inline bool SSLServer::process_and_close_socket(socket_t sock)
{
    auto ssl = detail::ssl_new(
        ...
        ctx_,
        ctx_mutex_,
        [&](SSL* ssl) { return detail::ssl_connect_or_accept_nonblocking(sock, ssl, SSL_accept, read_timeout_sec_, read_timeout_usec_); },
        [](SSL* /*ssl*/) { return true; });
...
inline bool SSLClient::initialize_ssl(Socket& socket, Error& error)
{
    auto ssl = detail::ssl_new(
        ...
        [&](SSL* ssl) {
            ...
            if (!detail::ssl_connect_or_accept_nonblocking(socket.sock, ssl, SSL_connect, connection_timeout_sec_, connection_timeout_usec_)) {
...
inline Client::Client(const char* scheme_host_port) : Client(scheme_host_port, std::string(), std::string()) {}

inline Client::Client(const char* scheme_host_port, const std::string& client_cert_path, const std::string& client_key_path)
{
