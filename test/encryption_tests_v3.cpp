...
TEST(EncryptDataTestsV3, MultipleSiteKeys)
{
	...
	const std::vector<Key> keys = {
		Key{0, SITE_ID, (int)NAN, NOW, NOW.AddDays(3), NOW.AddDays(10), MakeKeySecret(0)},
		Key{1, SITE_ID, (int)NAN, NOW, NOW.AddDays(-4), NOW.AddDays(10), MakeKeySecret(1)},
		Key{2, SITE_ID, (int)NAN, NOW, NOW.AddDays(-2), NOW.AddDays(10), MakeKeySecret(2)},
		Key{3, SITE_ID, (int)NAN, NOW, NOW.AddDays(-4), NOW.AddDays(-3), MakeKeySecret(3)},
		Key{4, SITE_ID, (int)NAN, NOW, NOW.AddDays(-4), NOW.AddDays(1), MakeKeySecret(4)},
		Key{5, SITE_ID, (int)NAN, NOW, NOW.AddDays(-5), NOW.AddDays(2), MakeKeySecret(5)},
		Key{6, SITE_ID, (int)NAN, NOW, NOW.AddDays(-1), NOW.AddDays(2), MakeKeySecret(6)}
	};
...
TEST(EncryptDataTestsV3, KeyExpired)
{
	const std::uint8_t data[] = {1, 2, 3, 4, 5, 6};
	UID2Client client("ep", "ak", CLIENT_SECRET, IdentityScope::UID2);
	const Key key{SITE_KEY_ID, SITE_ID, (int)NAN, NOW, NOW, NOW.AddDays(-1), GetSiteSecret()};
...
TEST(EncryptDataTestsV3, KeyInactive)
{
	const std::uint8_t data[] = {1, 2, 3, 4, 5, 6};
	UID2Client client("ep", "ak", CLIENT_SECRET, IdentityScope::UID2);
	const Key key{SITE_KEY_ID, SITE_ID, (int)NAN, NOW, NOW.AddDays(1), NOW.AddDays(2), GetSiteSecret()};
	const auto encrypted = client.EncryptData(EncryptionDataRequest(data, sizeof(data)).WithKey(key));
	EXPECT_FALSE(encrypted.IsSuccess());
	EXPECT_EQ(EncryptionStatus::KEY_INACTIVE, encrypted.GetStatus());
}
